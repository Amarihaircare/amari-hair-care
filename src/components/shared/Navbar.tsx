...
export default function Navbar() {
  ...
  function handleSearchSubmit(value: string) {
    const kwd = value.trim();
    if (!kwd) return;
    router.push(`/catalogue?search=${encodeURIComponent(kwd)}`);
    setShowSearch(false);
  }
  ...
  return (
    ...
    >
      <div className="relative flex w-full flex-wrap items-center justify-between bg-inherit px-4 py-4 md:max-w-[95%] md:flex-nowrap lg:p-0 lg:pt-8 xl:max-w-screen-xl">
        ...
        >
          <ul className="header_nav-list flex flex-col gap-4 px-2 py-8 lg:flex-row lg:items-center lg:justify-start lg:gap-10 lg:p-0">
                ...
                <RoundedIconButton
                  ...
                  className={cn({
                    "hidden lg:inline-flex":
                      action.name.toLowerCase() === "search",
                  })}
                  icon={action.icon}
                  data-search={action.name.toLowerCase() === "search"}
                />

                {action.name.toLowerCase() === "cart" && cartCount > 0 && (
