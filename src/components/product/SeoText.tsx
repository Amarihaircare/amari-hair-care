        ...
        {seoText.map((text, index) => (
          <div key={index} className="flex flex-col gap-2">
            ...
            {text.description.map((description, index) => (
              <PurifiedHtml html={description} key={index} />
            ))}
          </div>
        ))}
      </div>
      <div className="flex flex-col gap-2">
        <h2 className="text-2xl font-bold">{en.amariDifferenceTitle}</h2>
        <p className="text-sm">{en.amariDifferenceLineOne}</p>
        <PurifiedHtml html={en.amariDifferenceLineTwo} />
      </div>
